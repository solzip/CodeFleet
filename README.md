...
에이전트에 관심이 없어도 가져갈 게 셋 있다 — **멱등 키를 호출자가 아니라 의미에서 뽑는 것**, **아무것도 안 본 검사를 통과가 아니라 실패로 다루는 것**, **상태 변경에서 커밋하는 지점을 하나만 두고 이름을 붙이는 것.** 셋 다 아래 「무엇을 알아냈나」에 근거와 함께 있다 — 첫째는 코드로, 둘째는 그 원칙이 실제로 잡아낸 조용한 초록 2건으로, 셋째는 여덟 단계의 이름으로. 이 프로젝트가 아니었어도 성립하는 이야기다.
