...
- **The suite is green on Windows and red on Linux.** A CI workflow added on the last day ran and failed on both platforms — six tests on Linux, two on Windows — while the same commit passes locally. One Linux failure is a POSIX behaviour this archive had predicted and never measured; the two Windows failures are tests added on that same last day. The workflow has since been removed, because an archived repository has nobody to act on a red check and it cannot be made green without doing the work this archive declined. The failures, with run ids, are recorded in [`docs/runs/2026-08-13/ci-first-run.md`](docs/runs/2026-08-13/ci-first-run.md). Nothing here was exercised under repetition or concurrency.
