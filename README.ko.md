...
> 실험이었다. 2026-08-13에 결과와 그 대가를 남기고 아카이브했다.
...
| — | 정책은 `meet`으로만 합성되고 역할은 상한만 기여한다. 좁혀지는 건 관측됐지만, 넓히려는 시도를 거부하는 건 테스트에만 있다 | 코드만 |

정리하다 보니 **둘이 사실 같은 이야기**였다. 권한 등급과 gap/defect 구분은 둘 다 **"모른다는 것을 데이터로 어떻게 표현하나"**에 대한 답이다. `LESSONS.md`에 정리한 결함 유형 일곱 가지 중 셋도 뿌리가 같다. **없음과 값을 구분하지 않은 것.** 이 프로젝트가 실제로 힘을 쓴 문제는 AI 검증이 아니라 **부재를 어떻게 표현하느냐**였다.
...
- `npm test`는 종료 코드 **0**이다(257 통과, 0 실패). 조건 커버리지는 545줄 중 345줄, 63.3%다. 이건 통과한 테스트가 그만큼의 줄을 인용했다는 뜻이지 **그 조건들이 제대로 구현됐다는 뜻이 아니다.**
- **스위트는 Windows에서 초록이고 Linux에서 빨갛다.** 마지막 날 추가한 CI 워크플로가 실제로 돌았고 양쪽에서 실패했다 — Linux 6건, Windows 2건. 같은 커밋이 로컬에서는 통과한다. Linux 실패 하나는 이 아카이브가 **예측만 하고 실측하지 못했던 POSIX 거동**이고, Windows 실패 둘은 같은 마지막 날에 추가한 테스트다. 워크플로는 이후 제거했다. 아카이브된 저장소에는 빨간 체크를 보고 조치할 사람이 없고, 이 아카이브가 하지 않기로 한 작업 없이는 초록으로 만들 수도 없기 때문이다. 실패 내역은 run id와 함께 [`docs/runs/2026-08-13/ci-first-run.md`](docs/runs/2026-08-13/ci-first-run.md)에 남아 있다. 반복 실행과 동시성에서 시험한 것은 여전히 하나도 없다.
