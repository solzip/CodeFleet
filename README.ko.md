...
- 마지막 날 추가한 CI 워크플로가 **이제 돌았고, 양쪽 플랫폼에서 실패했다.** 같은 커밋이 로컬에서는 초록인데 Linux에서 6건, Windows에서 2건이 깨진다. Linux 실패 하나는 이 아카이브가 **예측만 하고 실측하지 못했던 POSIX 거동**이고, Windows 실패 둘은 마지막 날 추가한 테스트다. 상세: [`docs/runs/2026-08-13/ci-first-run.md`](docs/runs/2026-08-13/ci-first-run.md). 반복 실행과 동시성에서 시험한 것은 여전히 하나도 없다.
