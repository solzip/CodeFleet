...
바로 다음 논의 주제는 Review model v0.2 구현 세부다.
```
...
v0.1 / v0.2 / final implementation slicing도 고정했다. v0.2는 final boundary를 약화하지 않고 unavailable / degraded / migration input으로 미구현 책임을 표현한다.
다음 병목은 Review model v0.2 구현 세부다.
