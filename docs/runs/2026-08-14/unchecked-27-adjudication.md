...
> **각주 (같은 날 추가)** — 아래 두 항목은 **이 문서를 쓴 뒤 한 시간쯤 지나 해소됐다.** 「파일:라인 검사기를 만들지 않았다」와 「감사 문서의 인용 100여 건은 대조하지 않았다」가 그것이고, `runs/2026-08-14/citation-and-denominator-checks.md`가 `scripts/check-doc-citations.mjs`를 만들어 인용 248건을 대조하고 감사 문서의 242건을 **검증 불가**(대상 커밋이 히스토리 재작성으로 사라짐)로 보고한다. 규약 §7에 따라 아래 원문은 고치지 않고 이 각주가 정정 사실을 보유한다.
>
> 이 각주가 필요했다는 것 자체가 이 문서가 지적한 실패의 재현이다 — **판정은 옮겨졌는데 그것을 적어둔 문서는 몰랐다.**
