...
> **각주 (2026-08-13 이후 추가)** — 「유일한」은 더 이상 참이 아니다. 이후 링크 체커(`scripts/check-links.mjs`)와 그 실패 테스트가 요청에 따라 추가돼 두 번째 비문서 변경이 됐다. 원문은 규약 §7에 따라 고치지 않는다 — `docs/runs/2026-08-13/link-checker-in-repo.md`.
