...
>
> **동결 이후 판정 1건** — `runs/2026-08-14/unchecked-27-adjudication.md`가 [미확인] 27건을 전수 판정했다. 등재부의 상태·건수는 동결 규칙에 따라 바뀌지 않았고, 판정은 그 문서가 보유한다.
...
| 날짜 | 문서 | 유형 | 결론(한 줄) | 후속 |
| --- | --- | --- | --- | --- |
| 08-14 | `runs/2026-08-14/citation-and-denominator-checks.md` | 수정 | **세 번째 검사기와 그 앞의 측정.** 살아 있는 문서의 수치 주장 567개 중 앵커는 34개(6.0%)였고 검사기가 분자만 보고하고 있었다 — 이제 미검사 잔여를 함께 낸다. `check-doc-citations.mjs`가 **네 번째 드리프트**(`LESSONS.md`)를 찾고, 2026-08-10 감사 인용 242건이 **히스토리 재작성으로 영구 검증 불가**임을 드러냈다. 테스트 건수 앵커는 붙자마자 291→318을 정정시켰다 | 종결 |
| 08-14 | `runs/2026-08-14/unchecked-27-adjudication.md` | 감사 | **[미확인] 27건 전수 판정.** 21건이 유효, 4건은 조건이 사라져 있었다 — 해소 3(P1-9·16·36) / 무효화됨 1(P1-14, **2026-08-12에 이미 판정돼 있었다**) / 부분해소 2(P1-5·39) / 미해소 20 / 유효 확정 1(P1-21). 등재부의 파일:라인 근거 2건이 어긋난 것도 함께 발견 | 종결 (동결 유지) |

## 2026-08-13

| 날짜 | 문서 | 유형 | 결론(한 줄) | 후속 |
| --- | --- | --- | --- | --- |
| 08-13 | `runs/2026-08-13/prose-fact-check.md` | 수정 | **산문 검사기**(`check-doc-facts.mjs`)를 만들어 `npm test`가 소비. 문서가 `<!-- fact: name = value -->`로 주장을 선언하면 실측과 대조한다 — 선언 34 · 실측 15종 · 불일치 0. 직전 검토의 미해소 5건 중 **4건 해소** | 종결 |
| 08-13 | `runs/2026-08-13/full-review-v2.md` | 감사·수정 | 전체 검토 2회차. **표지에서 틀린 문장 5종** — 그중 둘은 저장소에 유리한 쪽으로 틀렸다(미확인 27건 서술이 P1-21 확인을 반영 안 함, CI 실패를 Linux 6건만 적고 Windows 2건 누락). 수치는 전부 일치 | 종결 |
| 08-13 | `runs/2026-08-13/link-checker-in-repo.md` | 수정 | 링크 체커를 `scripts/check-links.mjs`로 저장소에 넣고 **`posttest`가 소비**하게 했다. 실패 테스트 16건 포함, `npm test` **종료 코드 0**(273 통과). **동결 이후 두 번째 비문서 변경** — 요청에 따른 예외이며 이 변경이 거짓으로 만든 문장 7곳을 함께 고쳤다 | 종결 |
| 08-13 | `runs/2026-08-13/link-audit-full.md` | 감사·수정 | 추적 마크다운 **69개 전수** 링크 검사(경로·**대소문자**·앵커). **직전 문서의 「깨짐 0건」이 거짓이었다** — 3건이 깨져 있었고 검사기가 루트 기준 대체 탐색으로 통과시켰다. 3건 고쳐 **PASS** | 종결 |
| 08-13 | `runs/2026-08-13/readme-language-swap.md` | 수정 | 기본 표지를 **한국어로** 되돌리고, 보존본 `README.original.md`의 깨진 내부 링크를 대상만 고쳐 해소. 전수 검사가 **색인 자신의 누락 1건**(`archive/first-thinking.md`)과 검사기 결함을 함께 잡았다. 최종 **등재 52 = 실측 52 · 실링크 34건 깨짐 0건**. 실행 코드 무변경 | 종결 |
| 08-13 | `runs/2026-08-13/ci-first-run.md` | 감사·결정 | **CI가 실행됐고 양쪽 플랫폼에서 실패했다**(ubuntu 6 / windows 2). P1-21이 예측대로 실현됨을 실측 확인. 아카이브의 "한 번도 안 돌았다" 서술을 정정하고, **워크플로를 제거**했다 — 아카이브에는 빨간 체크를 읽을 사람이 없다 | 종결 |
| 08-13 | `archive/2026-08-13/DESIGN-NOTES.md` | 결정 | 설계 결과 10건을 검증 상태와 함께. 실행 검증 8 / 코드만 1 / 부분 검증 1 | 상시 (아카이브) |
| 08-13 | `archive/2026-08-13/README.original.md` | 결정 | 종료 전 루트 README. 동작하는 제품으로서 이 프로젝트를 어떻게 설명했는지의 기록. 본문은 원문 그대로이고, **내부 링크 대상만 보존본 경로로 고쳤다** | 상시 (아카이브) |
| 08-13 | `archive/2026-08-13/README.en.original.md` | 결정 | 종료 전 영문 README. 위와 같은 성격 | 상시 (아카이브) |
| 08-13 | `archive/2026-08-13/ENVIRONMENT.md` | 결정 | Windows·git·spawn에서 실측된 사실. 해결 미확정 3·미검증 4·미실측 1 = 8건을 명시 | 상시 (아카이브) |
| 08-13 | `archive/2026-08-13/LESSONS.md` | 결정 | 판정된 50건에서 결함 유형 7개. [미확인] 27건은 분류하지 않음 | 상시 (아카이브) |
| 08-13 | `archive/2026-08-13/ARCHIVE.md` | 결정 | **프로젝트 종료.** 결함 77건(해소 25 / 미확인 27), `npm test` 0, fixture 완주·실제 프로젝트 미완주. 종료 사유 2가지 | 상시 (아카이브) |
| 08-13 | `runs/2026-08-13/stranded-findings-triage.md` | 감사 | 방치된 11건 중 **8건이 이미 해소돼 있었다**(`38cf9c9`·`5055cf3`). 여전히 유효 3건(P1-45·46·47). 미해소 전건을 4개 군으로 우선순위화 | **끊김** (수정 미착수) |
...
| 날짜 | 문서 | 유형 | 결론(한 줄) | 후속 |
| --- | --- | --- | --- | --- |
| 06-02 | `archive/first-thinking.md` | 결정 | 초기 브레인스토밍 기록. **설계의 source of truth가 아니다**(문서 스스로 그렇게 밝힌다). 현재 기준은 `concept-foundation.md` | 상시 (아카이브) |

> **이 절은 2026-08-13 이후 추가됐다.** 색인은 `docs/archive/`의 전 문서를 대상으로 한다고 선언해 놓고 이 파일을 빠뜨렸다. 저장소 안 어디에서도 참조되지 않아 드러나지 않았고, 표지 스왑 작업의 전수 검사에서 발견했다 (`runs/2026-08-13/readme-language-swap.md`).
...
> **2026-08-14 갱신** — 아래 표가 가리키던 [미확인] 27건은 `runs/2026-08-14/unchecked-27-adjudication.md`가 전수 판정했다. **미후속이라는 서술은 이제 등재부의 상태 칸에만 남아 있다**(동결 규칙에 따라 바꾸지 않았다). 표 본문은 동결 시점 그대로 둔다.
