...
작성 기준: 2026-08-18. 전수 64개 문서 (감사 36 · 실행 21 · 아카이브 7). <!-- fact: docs-indexed = 64 --> <!-- fact: docs-on-disk = 64 -->
...
| 날짜 | 문서 | 유형 | 결론(한 줄) | 후속 |
| --- | --- | --- | --- | --- |
| 08-18 | `runs/2026-08-18/readme-answers-four-questions.md` | 감사·수정 | **표지가 네 질문 중 「그후 어떻게」에만 답하지 못했다.** 절별 행 수를 네 질문에 배정하니 앞의 둘이 표지의 대부분을 쓰는 동안 「그후」는 제목과 일곱 줄이 전부였고, 그 문단 셋이 전부 과거여서 **미래 문장이 없었다.** 남은 것·안 만들어도 되는 것·**그래도 남는 것**을 더했다 — 하네스가 가져간 건 배관이고 **판정 계층은 비어 있다**는 것이 아카이브를 납득시키면서 다음을 가리키는 문장이다. 근거가 저장소 전체에 없던 배너의 `PTY` 줄은 실측된 문장(`src/agent.ts:435`)으로 바꿨다 | 종결 (읽기 전용 복귀) |
| 08-18 | `runs/2026-08-18/version-claim-verified.md` | 감사 | **두 번 [미검증]으로 이월한 `v2.1.49`는 검증 가능했다** — 잘린 것은 CHANGELOG가 아니라 그것을 읽던 도구였고, 전문을 그대로 받으니 `--worktree` 플래그와 서브에이전트 worktree 격리를 추가한 판이 바로 거기였다(생성·정리 훅은 다음 판). **「0건 검사는 통과가 아니다」와 같은 모양** — *다 봤는데 없다*와 *내가 본 범위에 없다*가 같은 값으로 수렴했고, 도구가 분모를 보고했다면 첫 판정에서 잡혔다. 표지 문장은 맞으므로 고치지 않고 판정만 옮겼다 | 종결 (읽기 전용 복귀) |
| 08-18 | `runs/2026-08-18/license-badge-claim.md` | 감사·수정 | **"사이드바에 라이선스 배지가 뜨지 않는다"가 틀렸다.** 항목은 뜬다 — 안 붙는 것은 이름이고, GitHub이 파일을 인식하되 표준 라이선스로 식별하지 못해 `NOASSERTION`으로 둘 뿐이다. 저장소가 줄곧 `PRIVATE`이라 **아무도 그 문장을 잰 적이 없었다.** 사본 여섯 중 살아 있는 문서 셋을 고치고, 동결된 원본 둘과 날짜 붙은 실행 기록 하나는 규약대로 남겼다. **`LICENSE`는 안 건드렸다** — 배지에 이름을 띄우는 유일한 방법이 공개 제약이 금지하는 그 행위다 | 종결 (읽기 전용 복귀) |
| 08-18 | `runs/2026-08-18/public-and-archive.md` | 실행 | **표지가 말하는 상태와 플랫폼이 말하는 상태가 달랐다** — 두 표지가 동결 이후 줄곧 "아카이브, 이슈·PR 안 받음"이라고 적는 동안 GitHub 저장소는 `PRIVATE` + 활성이었다. 공개 전에 원격을 실측해 **재작성 이전 아이덴티티 0건 · 태그 0 · 실제 시크릿 0**을 확인하고 `PUBLIC`으로 켠 뒤 읽기 전용으로 아카이브했다. 아카이브가 마지막 행위라 **`isArchived`의 확인만은 이 저장소 안에 담길 자리가 없고, 그 사실을 값으로 적었다** | 종결 (읽기 전용) |
| 08-18 | `runs/2026-08-18/archive-banner.md` | 수정 | **표지에 종료 사유 배너.** 하네스가 worktree와 훅을 네이티브로 갖게 되면서 무효화된 전제 둘을 두 README 상단에 적고, **배너와 반대로 "다시 세워 시작했다"고 말하던 「후속」절을 같이 고쳤다**. 배너의 숫자 둘 중 훅 이벤트 개수는 설치된 바이너리에서 실측했고 버전은 **미검증으로 남겼다**. 작업 전 `npm test`가 이미 **종료 코드 1**이었는데, 원인은 배너가 아니라 **이 저장소가 "존재하지 않는다"고 판정해 둔 `fixes/env-DESKTOP-ENGO922.md`가 미추적으로 트리에 있던 것**이었다 — 트리 밖으로 옮겨 0을 확인했다 | 종결 (동결 유지) |

## 2026-08-14

| 날짜 | 문서 | 유형 | 결론(한 줄) | 후속 |
| --- | --- | --- | --- | --- |
| 08-14 | `runs/2026-08-14/freeze-notice.md` | 감사·수정 | **동결을 선언한 사람이 동결을 선언하지 않은 문서를 남겼다.** 검사기는 작업 전에 이미 전부 초록이었고, 정리가 필요한 것은 검사기가 보지 않는 쪽이었다 — `session-handoff.md`·`design-progress.md`가 배너 없이 「다음 슬라이스」를 지시하고 있었고 `CLAUDE.md`의 **`## Where to resume`이 목록 맨 앞 두 줄로** 그 둘을 가리켰다. 셋 다 고쳤다. `INDEX.md:5`의 커밋 해시는 **자기참조라 값으로 고칠 수 없어** 문장에서 뺐다 | 종결 (동결 유지) |
...
> **2026-08-14 갱신 — 첫 항목은 확인됐다.** 두 문서 모두 2026-08-13 작업을 반영하지 않고 있었다(`session-handoff.md`는 2026-08-11, `design-progress.md`는 2026-08-10에서 멈춘 채 「다음 슬라이스」를 지시하고 있었다). 각 문서 상단에 동결 배너를 붙였다 — `runs/2026-08-14/freeze-notice.md`. **나머지 두 항목은 미확인인 채로 둔다.** 아래 목록은 조사 시점 그대로다.
