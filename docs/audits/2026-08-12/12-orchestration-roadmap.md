...
- [x] **S5-0 · 결정** — **(a) 명시적 `codefleet apply <run-id>`로 확정** (사용자 결정, 2026-08-12)
  - 근거: ACCEPTED 리뷰가 자동 반영하면 리뷰 결정과 워크스페이스 변경이 한 행위로 묶이고, 격리가 지키려던 결정이 사라진다
- [x] **S5-1 · P1-27** — 반영 경로 구현 (`src/apply.ts`)
  - 적용 대상은 격리 트리가 아니라 **Harness가 관측한 diff**다. 트리는 Run 종료 시 폐기되므로, 그 사이 변했을 수 있는 디렉터리가 아니라 증거를 적용한다
  - 패치를 믿을 수 없는 모든 사유가 **부분 적용이 아니라 거부**다: 수용 리뷰 없음 / diff 잘림(waive 불가) / 격리 없이 실행 / 워크스페이스가 변함
  - 충돌은 `git apply --check`가 판정하고 거부문이 **git 자신의 출력**을 옮긴다. 요약하면 어느 hunk가 안 맞았는지 사라진다
  - `--check`는 뮤테이션과 **같은 planner**를 쓴다. 미리보기를 따로 구현하면 다른 것의 미리보기가 된다
  - `RUN_RESULT_APPLIED`를 Objective 원장에 append. 재적용은 두 번 적용이 아니라 no-op
- [x] **S5-2** — 반영 후 다음 Task가 이전 결과 위에서 시작하는지 확인
  - 확인됨: 반영·커밋 후 같은 계약을 다시 실행하면 두 번째 Run의 diff가 **비어 있다**. 첫 결과 위에서 시작했다는 증거다
- [x] **S5-3** — 회귀 테스트 (`test/apply.test.ts` 5건)
  - **구현 중 확인한 사실 2가지**: REJECTED 리뷰는 `DEGRADED_RECORDED`가 되어 import 자체가 거부되므로, Objective 원장에는 애초에 수용된 결정만 들어간다. 따라서 "리뷰됐지만 수용 아님"은 apply에 **결정 없음**으로 도달한다
  - 결정 검사가 내용 검사보다 **앞**이다. 변경 없는 미리뷰 Run은 "아무도 결정하지 않았다"로 거부된다 — 그쪽이 더 쓸모 있는 정보다

### S6 — 편의·확장 (결함 아님)

- [x] **S6-1** — `objective next` — 다음 실행 대상을 보여준다
  - NEXT가 없을 때 **왜 없는지**를 적는다. 리뷰 대기와 완료는 다른 상태인데 침묵하면 같아 보인다
- [x] **S6-2** — `objective run-next` — 1회 실행이며 루프가 아니다
  - 실행 후 "이건 한 항목만 실행하고 멈췄다. 큐가 움직이려면 리뷰해라"를 출력한다
  - **구현 중 발견**: Run은 리뷰가 등재되기 전까지 큐에 아무 흔적도 남기지 않아서, 이미 실행한 항목과 시작도 안 한 항목이 구별되지 않는다. `next`/`run-next`가 기존 Run 목록을 함께 출력하도록 했다. 이게 없으면 `run-next` 반복이 같은 항목을 조용히 다시 실행한다
- [x] **S6-3 · P1-35** — `run-record.md`가 실행한 검증 커맨드를 이름으로 적는다
  - argv 그대로, commandId·결과·exit code와 함께. 커맨드가 0개면 "이 게이트 결과는 실행된 커맨드에 근거하지 않는다"를 명시한다
  - 실제 사례가 근거다: Spring Boot 프로젝트 첫 실행에서 `gradle --version` 하나로 SATISFIED가 나왔고, 사람이 읽는 유일한 문서가 그 사실을 적지 않았다
- [x] **S6-4 · P1-34** — win32에서 빌드 wrapper를 부를 수 있게 한다
  - 배치 파일일 때만 Harness가 `shell`을 공급한다. Task가 `cmd`를 지목하는 길은 여전히 막혀 있다 — 인터프리터를 **계약에서 받지 않고 Harness가 준다**는 것이 차이다
  - cmd.exe가 문법으로 읽는 문자(`& | < > ^ " % ! 개행`)가 argv에 있으면 **거부한다.** 따옴표로 감싸서 통과시키면 정책 검사를 통과한 argv 배열이 다시 셸 문자열이 된다
  - 심사 로직은 플랫폼 무관하게 테스트한다(규칙이 본체다). 실제 실행 테스트만 win32 한정
- [x] **S6-5 · P1-43** — `resume.sourceHashPolicy`
  - 제거하지 않았다 — 설계가 Run Plan의 shape로 고정한 필드다. 대신 **선언을 검사 대상으로 만들었다**: `TASK_AND_PROFILE_MUST_MATCH`가 참인 이유(두 절반 각각의 거부)를 테스트가 붙든다
  - resume 기능 자체는 없으므로 그 기능에 의한 소비는 여전히 불가능하다. 아무도 검사하지 않는 선언은 JSON 안의 산문이고, 이 파일은 증거다
- [x] **S6-6 · B** — 두 번째 어댑터: **`claude` (Claude Code CLI)** [사용자 결정, 2026-08-12]
  - 어댑터 하나뿐일 때 "provider-agnostic"은 요구된 적 없는 코드에 대한 주장이었다. 두 번째를 붙여서 **실제로 공유되는 부분**(dry-run 단락, capability 거부, 프롬프트 읽기, 상한 있는 실행)을 하나로 모으고, 진짜 다른 것만 어댑터에 남겼다 — 기본 커맨드와 transcript 읽는 법
  - **이 작업이 S4의 실제 버그를 드러냈다.** `runAgentSafely`가 해석된 어댑터가 아니라 `config.agentAdapter`(프로파일 기본값)를 쓰고 있었다. Run Plan은 override를 기록하고 실행은 무시했다 — Run Trace가 한 어댑터를 적고 다른 어댑터가 돌았다. **어댑터가 하나일 땐 두 값이 항상 같은 문자열이라 드러날 수 없었다.** S4 테스트가 통과한 이유는 dry-run이라 어댑터를 아예 띄우지 않았기 때문이다
  - Claude transcript는 shell 호출이 assistant 메시지 안의 `tool_use` 블록이고 커맨드가 argv가 아니라 셸 문자열이다. **같은 규칙 적용**: 셸 문자열을 쪼개면 단어 경계를 지어내는 것이므로 raw만 보관하고 argv는 비운다. 그래서 어느 어댑터가 만들었든 provider 주장은 command policy를 만족시킬 수 없다
  - 인식하지 못하는 형식은 추측하지 않고 `PROVIDER_TRANSCRIPT_FORMAT_UNRECOGNIZED`로 격하한다
  - 기본 allowlist는 `["codex"]` 그대로다. allowlist는 결정이므로 새 워크스페이스가 두 번째 어댑터를 자동으로 허용하지 않는다
  - **테스트 작성 중 사고**: 로컬 오버레이를 손으로 잘못된 경로에 써서 실제 `claude` CLI가 임시 디렉터리에서 74초간 실행됐다. 오버레이가 안 읽히면 조용히 어댑터 기본 커맨드로 떨어진다. 픽스처 헬퍼를 쓰도록 고쳤고, 그 이유를 주석에 남겼다

### S7 — 전체 재검토

- [x] **S7-1** — S1~S6 완료 후 시스템 전체 검토 → **`13-system-review.md`**
  - 불변식 재판정: **7 위반 + 2 부분 → 1 위반**. 커밋을 읽지 않고 금지된 것을 실제로 시도해서 판정했다
  - 실사용 재시도: 차단 5개 중 **1개 해소(P1-34)**, **1개 이동(P1-32 — 늦은 침묵에서 이른 명시적 거부로)**, 설계대로인 2개는 그대로, 1개는 제품 결함 아님
  - **경로는 짧아지지 않고 길어졌다.** relation 부착과 프로파일 변경 후 재승인이 새로 필요하다. 정의를 코드가 실제로 강제하면 단계가 는다는 사실을 그대로 적었다
  - 등재 대조: 누락 0. 이번 작업 중 신규 등재 3건(P1-44·45·46) 전부 SUMMARY에 있다
  - **검토가 결함 1건을 새로 찾았다**: 가드레일이 움직인 뒤 `draftState`가 `READY_FOR_APPROVAL`을 출력하는데 `approve`는 거부한다. S2에서 내용 절반에 대해 고친 결함이 S1-1이 나중에 추가한 가드레일 절반에 남아 있었다. 수정 + 회귀 고정
  - 남은 것: **M-6**(승격이 승인과 같은 뮤테이션, 로드맵 범위 밖) / P1-32 잔여(`init` 기본값) / P1-45 / P1-46 / 새 코드의 rule claim 미분류
