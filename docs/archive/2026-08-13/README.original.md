...
> 이 파일은 종료 전 루트 README의 보존본이다. 링크 대상만 보존본·저장소 루트 기준으로 고쳤고 본문은 원문 그대로다.
...
## 6.5 반영 — 격리 트리의 결과를 워크스페이스로

격리는 에이전트의 작업이 워크스페이스에 **자동으로 닿지 않게** 하려고 있다. 그래서 반영도 자동이 아니다.

```bash
codefleet apply 2026-05-27_001 --reason "수용했고, 워크스페이스로 옮긴다"
codefleet apply 2026-05-27_001 --check   # 아무것도 바꾸지 않고 적용 가능 여부만 판정
```

반영되는 것은 **Harness가 관측한 diff**다. 격리 트리는 Run이 끝나면 폐기되므로, 그 사이에 변했을 수 있는 디렉터리가 아니라 증거를 적용한다. 그래서 패치를 믿을 수 없는 모든 사유가 부분 적용이 아니라 **거부**다:

| 거부 | 이유 |
| --- | --- |
| 수용된 리뷰 없음 | 이 명령은 결정에 따라 행동한다. 결정이 없으면 행동할 것이 없다 |
| diff가 잘림 | 변경의 일부만 쓰는 것은 아무것도 안 쓰는 것보다 나쁘다. waive 불가 |
| 격리 없이 실행됨 | 이미 워크스페이스에 있다. 옮길 것이 없다 |
| 워크스페이스가 변함 | 패치가 기술하는 내용이 거기 없다. 어느 쪽이 이기는지 정하는 것은 이 도구의 일이 아니다 |

충돌은 `git apply --check`가 판정하고, 거부문에 **git 자신의 출력**을 옮긴다. 거부는 워크스페이스를 건드리지 않는다. 반영되면 `RUN_RESULT_APPLIED`가 Objective 원장에 append되고, 같은 Run을 다시 반영하는 것은 두 번 적용이 아니라 no-op이다.

**설계 문서에 이 경로에 대한 규정이 없다.** 명시적 커맨드를 택한 이유는, ACCEPTED 리뷰가 자동 반영하면 리뷰 결정과 워크스페이스 변경이 한 행위로 묶이고 격리가 지키려던 결정이 사라지기 때문이다.
